...
export class AddTypeComponent {
  ...
  addModType(typeRecibido: PetType) {
    console.log(typeRecibido);
    let copyType: PetType;
    if(typeRecibido.id){
      this.typeService.modType(typeRecibido).subscribe({
        next: result => {
          console.log(result);
          copyType = typeRecibido;
          this.pafuera2.emit()
        },
        error: err => console.error(err)
      });
    } else {
      this.typeService.insertType(typeRecibido).subscribe({
        next: result => {
          console.log(result);
